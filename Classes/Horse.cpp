...
{
	Head = Head->InitAnimation(scene, 0.07f, "horse/horse_head.txt");

	Head->setAnchorPoint(Vec2(0.5f, 0));
	Head->setPosition(Vec2(640, 0));

	Legs = Legs->InitAnimation(scene, 0.07f, "horse/horse_legs.txt");

	Legs->setAnchorPoint(Vec2(0.5f, 0));
	Legs->setPosition(Vec2(640, 0));

	Body = Body->InitAnimation(scene, 0.07f, "horse/horse_body.txt");
