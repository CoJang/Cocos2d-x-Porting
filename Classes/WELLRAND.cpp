...
static void InitWELLRNG19937a(unsigned int *init) {
	int j;
	state_i = 0;
...
void InitRand() {
	...
	for (unsigned long i = 1; i < R; ++i) {
		for (unsigned long j = 0; j < i; ++j) {
			...
			while (_state[i] == _state[j]);
		}
	}
	InitWELLRNG19937a(_state);
}

unsigned long long MakeRand() {
	return WELLRNG19937a() * 1.0e+8;
}

unsigned long long MakeRandWithMax(unsigned long long MaxRange)
{
	return MakeRand() % (MaxRange + 1u);
}

unsigned long long MakeRandWithRange(unsigned long long MinRange, unsigned long long MaxRange)
{
	unsigned long long temp = MaxRange - MinRange + 1u;

	temp = (MakeRand() % temp) + MinRange;
